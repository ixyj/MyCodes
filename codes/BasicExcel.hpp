// Accsee the Excel file
//Modified by Xu Yajun, 2010.10

/********************************************************************************************************
===========class BasicExcel=============
void Create(int sheets=3) Create a new Excel workbook with a given number of spreadsheets (Minimum 1). 
bool Open(const char* filename) Open an Excel workbook from a file. 
...
protected:
	...
	public:
		...
		void Write(char* block);
		void Read(char* block);
		friend bool operator==(const CCompoundFile::CProperty& lhs, const CCompoundFile::CProperty& rhs) 
		{
		...
		friend bool operator!=(const CCompoundFile::CProperty& lhs, const CCompoundFile::CProperty& rhs) {return !(lhs == rhs);}
		friend bool operator > (const CCompoundFile::CProperty& lhs, const CCompoundFile::CProperty& rhs) {return (rhs < lhs);}
		friend bool operator<=(const CCompoundFile::CProperty& lhs, const CCompoundFile::CProperty& rhs) {return !(rhs < lhs);}
		friend bool operator>=(const CCompoundFile::CProperty& lhs, const CCompoundFile::CProperty& rhs) {return !(lhs < rhs);}
	...
	public:
		CPropertyTree();
		~CPropertyTree();
		CPropertyTree* parent_;
		CProperty* self_;
		size_t index_;
		vector<CPropertyTree*> children_;
	};
	void LoadProperties();
	void SaveProperties();
	int MakeProperty(const wchar_t* path, CProperty* property);
	CPropertyTree* FindProperty(size_t index);
	CPropertyTree* FindProperty(const wchar_t* path);
	CPropertyTree* FindProperty(CPropertyTree* parentTree, wchar_t* name);
	void InsertPropertyTree(CPropertyTree* parentTree, CProperty* property, size_t index);
	void DeletePropertyTree(CPropertyTree* tree);
	void UpdateChildrenIndices(CPropertyTree* parentTree);
	void IncreasePropertyReferences(CPropertyTree* parentTree, size_t index);
	void DecreasePropertyReferences(CPropertyTree* parentTree, size_t index);
	CPropertyTree* propertyTrees_;
	CPropertyTree* currentDirectory_;
	vector<CProperty*> properties_;
	vector<CPropertyTree*> previousDirectories_;
};
...
public:
	...
	struct Palette;
	struct UseSelfs;
	struct BoundSheet : public CRecord
	{
		BoundSheet();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);	
	...
	struct Country;
	struct LinkTable;
	struct SharedStringTable : public CRecord
	{
		SharedStringTable();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);	
...
public:
	...
	struct ColInfo;
	struct Sort;
	struct Dimensions : public CRecord
	{
		Dimensions();
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);	
	...
	struct CellTable
	{
		struct RowBlock
		{
			...
			struct CellBlock
			{
				...
				struct LabelSST : public CRecord
				{
					LabelSST();
					virtual size_t Read(const char* data);
					virtual size_t Write(char* data);	
				...
				struct MulBlank : public CRecord
				{
					MulBlank();
					virtual size_t Read(const char* data);
					virtual size_t Write(char* data);	
				...
				struct Formula : public CRecord
				{
					struct Array : public CRecord
					{
						Array();
						virtual size_t Read(const char* data);
						virtual size_t Write(char* data);	
					...
					struct ShrFmla1 : public CRecord
					{
						ShrFmla1();
						virtual size_t Read(const char* data);
						virtual size_t Write(char* data);	
...
class CBasicExcel;
class CBasicExcelWorksheet;
class CBasicExcelCell;
...
public:
	CBasicExcel();
	CBasicExcel(const char* filename);
	CBasicExcel(const wchar_t* filename);
	~CBasicExcel();

public: // File functions.
	void Create(int sheets = 3);	///< Create a new Excel workbook with a given number of spreadsheets (Minimum 1).
	bool Open(const char* filename);	///< Load an Excel workbook from a file.
...
public: // Worksheet functions.
	...
	CBasicExcelWorksheet* GetWorksheet(size_t sheetIndex);	///< Get a pointer to an Excel worksheet at the given index. Index starts from 0. Returns 0 if index is invalid.
	CBasicExcelWorksheet* GetWorksheet(const char* name);	///< Get a pointer to an Excel worksheet that has given ANSI name. Returns 0 if there is no Excel worksheet with the given name.
	CBasicExcelWorksheet* GetWorksheet(const wchar_t* name);	///< Get a pointer to an Excel worksheet that has given Unicode name. Returns 0 if there is no Excel worksheet with the given name.
	
	CBasicExcelWorksheet* AddWorksheet(int sheetIndex=-1);	///< Add a new Excel worksheet to the given index. Name given to worksheet is SheetX, where X is a number which starts from 1. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CBasicExcelWorksheet* AddWorksheet(const char* name, int sheetIndex=-1);	///< Add a new Excel worksheet with given ANSI name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CBasicExcelWorksheet* AddWorksheet(const wchar_t* name, int sheetIndex=-1);	///< Add a new Excel worksheet with given Unicode name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
...
public:
	CCompoundFile file_;						///< Compound file handler.
	CWorkbook workbook_;						///< Raw Workbook.
	vector<CWorksheet> worksheets_;			///< Raw Worksheets.
	vector<CBasicExcelWorksheet> yesheets_;	///< Parsed Worksheets.
};

class CBasicExcelWorksheet
{
	friend class CBasicExcel;

public:
	CBasicExcelWorksheet(CBasicExcel* excel, size_t sheetIndex);
...
public: // Cell functions
	...
	CBasicExcelCell* GetCell(size_t row, size_t col); ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.
	bool EraseCell(size_t row, size_t col); ///< Erase content of a cell. row and col starts from 0. Returns true if successful, false if row or col exceeds range.
...
private:
	CBasicExcel* excel_;					///< Pointer to instance of BasicExcel.
	...
	vector<vector<CBasicExcelCell> > cells_;	///< Cells matrix.
};
...
public:
	...
	friend ostream& operator<<(ostream& os, const CBasicExcelCell& cell);	///< Print cell to output stream. Print a null character if cell is undefined.
...
bool CBlock::Create(const wchar_t* filename)
...
bool CBlock::Create(const char* filename)
{
...
bool CBlock::Open(const wchar_t* filename, ios_base::openmode mode)
...
bool CBlock::Open(const char* filename, ios_base::openmode mode)
{
	int fileNameLen = strlen(filename);
	filename_.assign(fileNameLen+1, '\0');
	copy(filename, filename+fileNameLen, filename_.begin());
...
bool CBlock::Read(size_t index, char* block)
...
bool CBlock::Write(size_t index, const char* block)
...
bool CBlock::Swap(size_t index1, size_t index2)
...
bool CBlock::Move(size_t from, size_t to)
...
bool CBlock::Insert(size_t index, const char* block)
...
bool CBlock::Erase(size_t index)
...
bool CBlock::Erase(vector<size_t>& indices)
...
void CCompoundFile::CHeader::Write(char* block)
...
void CCompoundFile::CHeader::Read(char* block)
...
void CCompoundFile::CHeader::Initialize()
{
...
CCompoundFile::CProperty::CProperty() :
	nameSize_(0),
...
void CCompoundFile::CProperty::Write(char* block)
...
void CCompoundFile::CProperty::Read(char* block)
...
CCompoundFile::CPropertyTree::CPropertyTree() {};
...
CCompoundFile::~CCompoundFile() {this->Close();}

/************************* Compound File Functions ***************************/
bool CCompoundFile::Create(const wchar_t* filename)
...
// PROMISE: Return true if file is successfully created and opened, false if otherwise.
{
	...
	// Set property tree
	propertyTrees_ = new CPropertyTree;	
...
bool CCompoundFile::Open(const wchar_t* filename, ios_base::openmode mode)
...
/************************* Directory Functions ***************************/
int CCompoundFile::ChangeDirectory(const wchar_t* path)
...
int CCompoundFile::MakeDirectory(const wchar_t* path)
...
int CCompoundFile::PresentWorkingDirectory(wchar_t* path)
...
int CCompoundFile::PresentWorkingDirectory(vector<wchar_t>& path)
// PURPOSE: Get the full path of the current directory in the compound file.
{
	previousDirectories_.push_back(currentDirectory_);
	path.clear();
...
int CCompoundFile::RemoveDirectory(const wchar_t* path)
...
{
	CPropertyTree* directory = FindProperty(path);
...
int CCompoundFile::DelTree(const wchar_t* path)
...
int CCompoundFile::DirectoryList(vector<vector<wchar_t> >& list, const wchar_t* path)
{
	previousDirectories_.push_back(currentDirectory_);
	if (path != 0)
	{
...
/************************* File Functions ***************************/
int CCompoundFile::MakeFile(const wchar_t* path)
...
int CCompoundFile::RemoveFile(const wchar_t* path)
...
int CCompoundFile::FileSize(const wchar_t* path, size_t& size)
...
// PROMISE: size will not be set if file is not present in the compound file.
{
	...
	CPropertyTree* property = FindProperty(path);
...
int CCompoundFile::ReadFile(const wchar_t* path, char* data)
...
int CCompoundFile::ReadFile(const wchar_t* path, vector<char>& data)
// PURPOSE: Read a file's data in the compound file.
// PROMISE: Returns the small blocks of data stored by the Root Entry if path = "\".
// PROMISE: data will not be set if file is not present in the compound file.
{
...
int CCompoundFile::WriteFile(const wchar_t* path, const char* data, size_t size)
...
int CCompoundFile::WriteFile(const wchar_t* path, const vector<char>& data, size_t size)
// PURPOSE: Write data to a file in the compound file.
// PROMISE: The file's original data will be replaced by the new data.
{
...
bool CCompoundFile::Create(const char* filename)
{
...
bool CCompoundFile::Open(const char* filename, ios_base::openmode mode)
{
	size_t filenameLength = strlen(filename);
	wchar_t* wname = new wchar_t[filenameLength+1];
	mbstowcs(wname, filename, filenameLength);
...
int CCompoundFile::ChangeDirectory(const char* path)
{
...
int CCompoundFile::MakeDirectory(const char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::PresentWorkingDirectory(char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
...
int CCompoundFile::PresentWorkingDirectory(vector<char>& path)
{
...
int CCompoundFile::RemoveDirectory(const char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::DelTree(const char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::MakeFile(const char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::RemoveFile(const char* path)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::FileSize(const char* path, size_t& size)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::ReadFile(const char* path, char* data)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::ReadFile(const char* path, vector<char>& data)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::WriteFile(const char* path, char* data, size_t size)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
int CCompoundFile::WriteFile(const char* path, vector<char>& data, size_t size)
{
	size_t pathLength = strlen(path);
	wchar_t* wpath = new wchar_t[pathLength+1];
	mbstowcs(wpath, path, pathLength);
...
void CCompoundFile::IncreaseLocationReferences(vector<size_t> indices)
...
// PROMISE: Changes will not be written to compound file.
{
	...
	// Change BAT Array references
	{
		for (size_t i=0; i<109 && header_.BATArray_[i]!=-1; ++i)
		{
			...
			for (size_t j=0; j<maxIndices; ++j)
			{
				if (header_.BATArray_[i] >= static_cast<int>(indices[j]) &&
	...
	if (header_.XBATCount_) 
	{
		...
		for (size_t j=0; j<maxIndices; ++j)
		{
			if (header_.XBATStart_ >= static_cast<int>(indices[j]) &&
	...
	if (header_.SBATCount_) 
	{
		...
		for (size_t j=0; j<maxIndices; ++j)
		{
			if (header_.SBATStart_ >= static_cast<int>(indices[j]) &&
	...
	{for (size_t i=0; i<maxBATindices && blocksIndices_[i]!=-1; ++i)
	{
		...
		for (size_t j=0; j<maxIndices; ++j)
		{
			if (blocksIndices_[i] > static_cast<int>(indices[j]) &&
				blocksIndices_[i] != -2 && 
	...
	{
		if (header_.propertiesStart_ >= static_cast<int>(indices[i]) &&
	...
	if (!properties_.empty()) 
	{
		...
		for (size_t j=0; j<maxIndices; ++j)
		{
			if (properties_[0]->startBlock_ >= static_cast<int>(indices[j]) &&
	...
	{for (size_t i=1; i<maxProperties; ++i)
	{
		if (properties_[i]->size_ >= 4096)
		{
			...
			for (size_t j=0; j<maxIndices; ++j)
			{
				if (properties_[i]->startBlock_ >= static_cast<int>(indices[j]) &&
...
void CCompoundFile::DecreaseLocationReferences(vector<size_t> indices)
...
void CCompoundFile::SplitPath(const wchar_t* path, 
							 wchar_t*& parentpath, 
...
size_t CCompoundFile::DataSize(size_t startIndex, bool isBig)
...
size_t CCompoundFile::ReadData(size_t startIndex, char* data, bool isBig)
...
size_t CCompoundFile::WriteData(const char* data, size_t size, int startIndex, bool isBig)
...
void CCompoundFile::GetBlockIndices(size_t startIndex, vector<size_t>& indices, bool isBig)
...
size_t CCompoundFile::GetFreeBlockIndex(bool isBig)
...
void CCompoundFile::ExpandBATArray(bool isBig)
...
void CCompoundFile::LinkBlocks(size_t from, size_t to, bool isBig)
...
void CCompoundFile::FreeBlocks(vector<size_t>& indices, bool isBig)
	...
	else
	{
		...
		{for (size_t i=0; i<maxIndices; ++i)
		{
			for (size_t j=0; j<maxSBATindices; ++j)
			{
				...
				if (sblocksIndices_[j] > static_cast<int>(indices[i]) &&
					sblocksIndices_[j] != -1 &&
...
void CCompoundFile::LoadProperties()
...
void CCompoundFile::SaveProperties()
...
int CCompoundFile::MakeProperty(const wchar_t* path, CCompoundFile::CProperty* property)
...
CCompoundFile::CPropertyTree* CCompoundFile::FindProperty(size_t index)
...
// PROMISE: is present, 0 if otherwise.
{
	...
	CPropertyTree* currentTree = previousDirectories_.back();
	if (currentTree->index_ != index)
	{
		...
		for (size_t i=0; i<maxChildren; ++i)
		{
			...
			CPropertyTree* child = FindProperty(index);
			if (child != 0) 
			{
				previousDirectories_.pop_back();
				return child;
...
CCompoundFile::CPropertyTree* CCompoundFile::FindProperty(const wchar_t* path)
...
// PROMISE: is present, 0 if otherwise.
{
	...
	if (parentpath != 0)
	{
		...
		if (ret != SUCCESS)
		{
			...
			currentDirectory_ = previousDirectories_.back();
			previousDirectories_.pop_back();
			CPropertyTree* property = NULL;
			return property;
		}
	}

	// Check to see if file is present in the specified directory.
	CPropertyTree* property = 0;
	if (filename != 0) 
	{
...
CCompoundFile::CPropertyTree*
CCompoundFile::FindProperty(CCompoundFile::CPropertyTree* parentTree, 
						   wchar_t* name)
...
void CCompoundFile::InsertPropertyTree(CCompoundFile::CPropertyTree* parentTree, 
									  CCompoundFile::CProperty* property, 
									  size_t index)
...
{
	CPropertyTree* tree = new CPropertyTree;
...
void CCompoundFile::DeletePropertyTree(CCompoundFile::CPropertyTree* tree)
...
void CCompoundFile::UpdateChildrenIndices(CCompoundFile::CPropertyTree* parentTree)
{
	// Update indices for 1st to middle child
	size_t maxChildren = parentTree->children_.size();
	if (maxChildren != 0)
	{
		vector<CPropertyTree*>& children = parentTree->children_;
		size_t prevChild = 0;
...
void CCompoundFile::IncreasePropertyReferences(CCompoundFile::CPropertyTree* parentTree, 
											  size_t index)
...
// EXPLAIN: because the index in property tree needs to be updated also.
{
	...
	{
		if (parentTree->self_->previousProp_ >= static_cast<int>(index))
		{
	...
	{
		if (parentTree->self_->nextProp_ >= static_cast<int>(index))
		{
	...
	if (parentTree->self_->childProp_ != -1)
	{
		if (parentTree->self_->childProp_ >= static_cast<int>(index))
		{
...
void CCompoundFile::DecreasePropertyReferences(CCompoundFile::CPropertyTree* parentTree, size_t index)
...
/************************************************************************************************************/
CRecord::CRecord() : dataSize_(0), recordSize_(4) {};
CRecord::~CRecord() {};
size_t CRecord::Read(const char* data)
{
...
size_t CRecord::Write(char* data)
{	
...
size_t CRecord::DataSize() {return dataSize_;}
size_t CRecord::RecordSize() {return recordSize_;}

/************************************************************************************************************/

/************************************************************************************************************/
BOF::BOF() : CRecord() {code_ = CODE::BOF; dataSize_ = 16; recordSize_ = 20;}
...
size_t BOF::Write(char* data)
{
	...
	return CRecord::Write(data);
}
/************************************************************************************************************/

/************************************************************************************************************/
YEOF::YEOF() : CRecord() {code_ = CODE::YEOF; dataSize_ = 0; recordSize_ = 4;}
/************************************************************************************************************/
...
size_t CWorkbook::Read(const char* data)
{
	size_t bytesRead = 0;
	short code;
...
size_t CWorkbook::Write(char* data)
{
...
size_t CWorkbook::RecordSize() {return DataSize();}
/************************************************************************************************************/

/************************************************************************************************************/
CWorkbook::Window1::Window1() : CRecord(), 
...
size_t CWorkbook::Window1::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorkbook::Window1::Write(char* data)
{
	data_.resize(dataSize_);
...
size_t CWorkbook::Font::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorkbook::Font::Write(char* data)
{
...
size_t CWorkbook::Font::DataSize() {return (dataSize_ = 14 + name_.RecordSize());}
size_t CWorkbook::Font::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/

/************************************************************************************************************/
CWorkbook::XF::XF() : CRecord(), 
...
size_t CWorkbook::XF::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorkbook::XF::Write(char* data)
{
	data_.resize(dataSize_);
...
size_t CWorkbook::Style::Write(char* data)
{
	data_.resize(DataSize());
...
size_t CWorkbook::Style::DataSize() {return (dataSize_ = (XFRecordIndex_ & 0x8000) ? 4 : 2+name_.RecordSize());}
size_t CWorkbook::Style::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/

/************************************************************************************************************/
CWorkbook::BoundSheet::BoundSheet() : CRecord(),
...
size_t CWorkbook::BoundSheet::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorkbook::BoundSheet::Write(char* data)
{
	data_.resize(DataSize());
...
size_t CWorkbook::BoundSheet::DataSize() {return (dataSize_ = 6+name_.RecordSize());}
size_t CWorkbook::BoundSheet::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/

/************************************************************************************************************/
CWorkbook::SharedStringTable::SharedStringTable() : CRecord(),
	stringsTotal_(0), uniqueStringsTotal_(0) {code_ = CODE::SST; dataSize_ = 8; recordSize_ = 12;}
size_t CWorkbook::SharedStringTable::Read(const char* data)
{
	CRecord::Read(data);
	...
	else
	{
		...
		for (int i=0, c=0; i<uniqueStringsTotal_; ++i)
		{
			...
			size_t multiplier = ((unicode & 1) != 0)? 2 : 1;
			if (static_cast<size_t>(c) >= maxContinue || npos+stringSize*multiplier+3 <= continueIndices_[c])
			{
			...
			else
			{
				...
				while (static_cast<size_t>(c) < maxContinue && npos+stringSize+1>continueIndices_[c])
				{
...
size_t CWorkbook::SharedStringTable::Write(char* data)
{
	data_.resize(DataSize());
	...
	for (int i=0, c=0, npos=8; i<uniqueStringsTotal_; ++i)
	{
		...
		if (static_cast<size_t>(c) < maxContinue && npos==continueIndices_[c]) ++c;
		else if (static_cast<size_t>(c) < maxContinue && static_cast<size_t>(npos) > continueIndices_[c])
		{
...
size_t CWorkbook::SharedStringTable::DataSize() 
{
	dataSize_ = 8;
	continueIndices_.clear();
	size_t curMax = 8224;
...
size_t CWorkbook::SharedStringTable::RecordSize()
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
/************************************************************************************************************/
CWorkbook::ExtSST::ExtSST() : CRecord(),
...
size_t CWorkbook::ExtSST::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorkbook::ExtSST::Write(char* data) 
{
	data_.resize(DataSize());
...
size_t CWorkbook::ExtSST::DataSize()
{
...
size_t CWorkbook::ExtSST::RecordSize() {return (recordSize_ = DataSize()+(int)((2+streamPos_.size()*8)/8224)*4)+4;}
/************************************************************************************************************/
...
size_t CWorksheet::Read(const char* data)
{
	size_t bytesRead = 0;
	short code;
	LittleEndian::Read(data, code, 0, 2);
...
size_t CWorksheet::Write(char* data)
{
	size_t bytesWritten = 0;
	bytesWritten += bof_.Write(data+bytesWritten);
...
size_t CWorksheet::RecordSize() {return DataSize();}
/************************************************************************************************************/

/************************************************************************************************************/
CWorksheet::Index::Index() : CRecord(), 
...
size_t CWorksheet::Index::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::Index::Write(char* data)
{
	data_.resize(DataSize());
...
size_t CWorksheet::Index::DataSize() {return (dataSize_ = 16 + DBCellPos_.size()*4);}
size_t CWorksheet::Index::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
/************************************************************************************************************/
CWorksheet::Dimensions::Dimensions() : CRecord(),
...
size_t CWorksheet::Dimensions::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::Dimensions::Write(char* data)
{
	data_.resize(DataSize());
...
/************************************************************************************************************/
CWorksheet::CellTable::RowBlock::CellBlock::Blank::Blank() : CRecord(), 
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0) {code_ = CODE::BLANK; dataSize_ = 6; recordSize_ = 10;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Blank::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Blank::Write(char* data)
{
	data_.resize(DataSize());
...
CWorksheet::CellTable::RowBlock::CellBlock::BoolErr::BoolErr() : CRecord(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), value_(0), error_(0)
	{code_ = CODE::BOOLERR; dataSize_ = 8; recordSize_ = 12;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::BoolErr::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::BoolErr::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, colIndex_, 2, 2);
...
CWorksheet::CellTable::RowBlock::CellBlock::LabelSST::LabelSST() : CRecord(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), SSTRecordIndex_(0)
	{code_ = CODE::LABELSST; dataSize_ = 10; recordSize_ = 14;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::LabelSST::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::LabelSST::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, colIndex_, 2, 2);
...
CWorksheet::CellTable::RowBlock::CellBlock::MulBlank::MulBlank() : CRecord(),
	rowIndex_(0), firstColIndex_(0), lastColIndex_(0)
	{code_ = CODE::MULBLANK; dataSize_ = 10; recordSize_ = 14;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulBlank::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulBlank::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulBlank::DataSize() {return (dataSize_ = 6 + XFRecordIndices_.size()*2);}
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulBlank::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}

CWorksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK::XFRK() :
	XFRecordIndex_(0), RKValue_(0) {};
void CWorksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK::Read(const char* data)
{
	LittleEndian::Read(data, XFRecordIndex_, 0, 2);
	LittleEndian::Read(data, RKValue_, 2, 4);
}	
void CWorksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK::Write(char* data)
{
	LittleEndian::Write(data, XFRecordIndex_, 0, 2);
	LittleEndian::Write(data, RKValue_, 2, 4);
}

CWorksheet::CellTable::RowBlock::CellBlock::MulRK::MulRK() : CRecord(),
	rowIndex_(0), firstColIndex_(0), lastColIndex_(0) {code_ = CODE::MULRK; dataSize_ = 10; recordSize_ = 14;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulRK::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, firstColIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulRK::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, firstColIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulRK::DataSize() {return (dataSize_ = 6 + XFRK_.size()*6);}
size_t CWorksheet::CellTable::RowBlock::CellBlock::MulRK::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}

CWorksheet::CellTable::RowBlock::CellBlock::Number::Number() : CRecord(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), value_(0) {code_ = CODE::NUMBER; dataSize_ = 14; recordSize_ = 18;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Number::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Number::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, colIndex_, 2, 2);
...
CWorksheet::CellTable::RowBlock::CellBlock::RK::RK() : CRecord(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), value_(0) {code_ = CODE::RK; dataSize_ = 10; recordSize_ = 14;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::RK::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::RK::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, colIndex_, 2, 2);
...
CWorksheet::CellTable::RowBlock::CellBlock::Formula::Formula() : CRecord(),
	rowIndex_(0), colIndex_(0), XFRecordIndex_(0), options_(0), unused_(0), type_(-1)
	{code_ = CODE::FORMULA; dataSize_ = 18; recordSize_ = 22;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
	LittleEndian::Read(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
	LittleEndian::Write(data_, colIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::DataSize() {return (dataSize_ = 18 + RPNtoken_.size());}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::RecordSize() 
{
	size_t dataSize = DataSize();
...
CWorksheet::CellTable::RowBlock::CellBlock::Formula::Array::Array() : CRecord(),
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Array::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Array::Write(char* data)
{
	data_.resize(DataSize());
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Array::DataSize() {return (dataSize_ = 12 + formula_.size());}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Array::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}

CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::ShrFmla() : CRecord(),
	firstRowIndex_(0), lastRowIndex_(0), firstColIndex_(0), lastColIndex_(0),
	unused_(0)
	{code_ = CODE::SHRFMLA; dataSize_ = 8; recordSize_ = 12;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, firstRowIndex_, 0, 2);
	LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, firstRowIndex_, 0, 2);
	LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::DataSize() {return (dataSize_ = 8 + formula_.size());}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}

CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::ShrFmla1() : CRecord(),
	firstRowIndex_(0), lastRowIndex_(0), firstColIndex_(0), lastColIndex_(0),
	unused_(0)
	{code_ = CODE::SHRFMLA1; dataSize_ = 8; recordSize_ = 12;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, firstRowIndex_, 0, 2);
	LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, firstRowIndex_, 0, 2);
	LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::DataSize() {return (dataSize_ = 8 + formula_.size());}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}

CWorksheet::CellTable::RowBlock::CellBlock::Formula::Table::Table() : CRecord(),
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Table::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, firstRowIndex_, 0, 2);
	LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::Table::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, firstRowIndex_, 0, 2);
	LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
CWorksheet::CellTable::RowBlock::CellBlock::Formula::String::String() : CRecord()
	{code_ = CODE::STRING; dataSize_ = 0; recordSize_ = 4;}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::String::Read(const char* data)
{
	CRecord::Read(data);
	string_.clear();
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::String::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::WriteString(data_, &*(string_.begin()), 0, string_.size());
	return CRecord::Write(data);
}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::String::DataSize() {return (dataSize_ = string_.size());}
size_t CWorksheet::CellTable::RowBlock::CellBlock::Formula::String::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
/************************************************************************************************************/
CWorksheet::CellTable::RowBlock::Row::Row() : CRecord(),
...
size_t CWorksheet::CellTable::RowBlock::Row::Read(const char* data)
{
	CRecord::Read(data);
	LittleEndian::Read(data_, rowIndex_, 0, 2);
...
size_t CWorksheet::CellTable::RowBlock::Row::Write(char* data)
{
	data_.resize(DataSize());
	LittleEndian::Write(data_, rowIndex_, 0, 2);
...
/************************************************************************************************************/
CWorksheet::CellTable::RowBlock::CellBlock::CellBlock() : 
	type_(-1), normalType_(true) {};
CWorksheet::CellTable::RowBlock::CellBlock::~CellBlock() {};
size_t CWorksheet::CellTable::RowBlock::CellBlock::Read(const char* data)
{
	size_t bytesRead = 0;
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::Write(char* data)
{
	size_t bytesWritten = 0;
	switch (type_)
	{
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::DataSize()
{
	switch (type_)
	{
		case CODE::BLANK:
...
size_t CWorksheet::CellTable::RowBlock::CellBlock::RecordSize() 
{
	switch (type_)
	{
		case CODE::BLANK:
...
short CWorksheet::CellTable::RowBlock::CellBlock::RowIndex()
{
	switch (type_)
	{
		case CODE::BLANK:
...
short CWorksheet::CellTable::RowBlock::CellBlock::ColIndex()
{
	switch (type_)
	{
		case CODE::BLANK:
...
/************************************************************************************************************/
CWorksheet::CellTable::RowBlock::DBCell::DBCell() : CRecord(),
	firstRowOffset_(0) {code_ = CODE::DBCELL; dataSize_ = 4; recordSize_ = 8;}
size_t CWorksheet::CellTable::RowBlock::DBCell::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::CellTable::RowBlock::DBCell::Write(char* data)
{
	data_.resize(DataSize());
...
size_t CWorksheet::CellTable::RowBlock::DBCell::DataSize() {return (dataSize_ = 4+offsets_.size()*2);}
size_t CWorksheet::CellTable::RowBlock::DBCell::RecordSize() 
{
	size_t dataSize = DataSize();
	return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
/************************************************************************************************************/
size_t CWorksheet::CellTable::RowBlock::Read(const char* data)
{
	size_t bytesRead = 0;
	short code;
	LittleEndian::Read(data, code, 0, 2);
...
size_t CWorksheet::CellTable::RowBlock::Write(char* data)
{	
	size_t bytesWritten = 0;
...
size_t CWorksheet::CellTable::RowBlock::DataSize() 
{
	size_t dataSize = 0;
	size_t maxRows = rows_.size();
...
size_t CWorksheet::CellTable::RowBlock::RecordSize() {return DataSize();}
/************************************************************************************************************/

/************************************************************************************************************/
size_t CWorksheet::CellTable::Read(const char* data)
{
	size_t bytesRead = 0;
...
size_t CWorksheet::CellTable::Write(char* data)
{	
	size_t bytesWritten = 0;
...
size_t CWorksheet::CellTable::DataSize() 
{
	size_t dataSize = 0;
	size_t maxRowBlocks_ = rowBlocks_.size();
...
size_t CWorksheet::CellTable::RecordSize() {return DataSize();}
/************************************************************************************************************/

/************************************************************************************************************/
CWorksheet::Window2::Window2() : CRecord(),
...
size_t CWorksheet::Window2::Read(const char* data)
{
	CRecord::Read(data);
...
size_t CWorksheet::Window2::Write(char* data)
{
	data_.resize(DataSize());
...
int GetRKValueFromDouble(double value)
{
	...
	int testVal1 = static_cast<int>(value);
	testVal1 *= 100;
	int testVal2 = static_cast<int>(value * 100);
	...
	int rkValue = static_cast<int>(intdouble.intvalue_);
	rkValue <<= 2;
...
{
	int testVal1 = static_cast<int>(value * 100);
	testVal1 *= 100;
	int testVal2 = static_cast<int>(value * 10000);
...
/************************************************************************************************************/
CBasicExcel::CBasicExcel() {};
CBasicExcel::CBasicExcel(const char* filename) 
{
	Open(filename);
}

CBasicExcel::CBasicExcel(const wchar_t* filename)
{
	Open(filename);
}
...
void CBasicExcel::Create(int sheets)
{
	workbook_ = CWorkbook();
	worksheets_.clear();
...
bool CBasicExcel::Open(const char* filename)
{
	if (file_.IsOpen()) file_.Close();
...
bool CBasicExcel::Open(const wchar_t* filename)
{
	if (file_.IsOpen()) file_.Close();
	if (file_.Open(filename))
	{
...
bool CBasicExcel::Save()
{
	if (file_.IsOpen())
	{
		...
		if (file_.WriteFile(L"Workbook", data, data.size())!=CCompoundFile::SUCCESS) return false;
		return true;
	}
	else return false;
}
...
bool CBasicExcel::SaveAs(const char* filename)
{
	if (file_.IsOpen()) file_.Close();

	if (!file_.Create(filename)) return false;
	if (file_.MakeFile("Workbook")!=CCompoundFile::SUCCESS) return false;

	return Save();
}

bool CBasicExcel::SaveAs(const wchar_t* filename)
{
	if (file_.IsOpen()) file_.Close();

	if (!file_.Create(filename)) return false;
	if (file_.MakeFile(L"Workbook")!=CCompoundFile::SUCCESS) return false;

	return Save();
}

// Total number of Excel worksheets in current Excel workbook.
size_t CBasicExcel::GetTotalWorkSheets()
{
...
CBasicExcelWorksheet* CBasicExcel::GetWorksheet(size_t sheetIndex)
{
...
CBasicExcelWorksheet* CBasicExcel::GetWorksheet(const char* name)
{
...
// Returns 0 if there is no Excel worksheet with the given name.
CBasicExcelWorksheet* CBasicExcel::GetWorksheet(const wchar_t* name)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
CBasicExcelWorksheet* CBasicExcel::AddWorksheet(int sheetIndex)
{
	size_t sheetNo = yesheets_.size() + 1;
	CBasicExcelWorksheet* yesheet = 0;
	do
	{
		char sname[50];
...
// Worksheet is added to the last position if sheetIndex == -1. 
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CBasicExcelWorksheet* CBasicExcel::AddWorksheet(const char* name, int sheetIndex)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
	...
	CWorkbook::BoundSheet* boundSheet;
	CWorksheet* worksheet;
	CBasicExcelWorksheet* yesheet;
	if (sheetIndex == -1)
	{
		workbook_.boundSheets_.push_back(CWorkbook::BoundSheet());
		worksheets_.push_back(CWorksheet());
		yesheets_.push_back(CBasicExcelWorksheet(this, worksheets_.size()-1));
	...
	{
		boundSheet = &*(workbook_.boundSheets_.insert(workbook_.boundSheets_.begin()+sheetIndex, CWorkbook::BoundSheet()));
		worksheet = &*(worksheets_.insert(worksheets_.begin()+sheetIndex, CWorksheet()));
		yesheet = &*(yesheets_.insert(yesheets_.begin()+sheetIndex, CBasicExcelWorksheet(this, sheetIndex)));
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CBasicExcelWorksheet* CBasicExcel::AddWorksheet(const wchar_t* name, int sheetIndex)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
bool CBasicExcel::DeleteWorksheet(size_t sheetIndex)
{
...
// Returns true if successful, false if otherwise.
bool CBasicExcel::DeleteWorksheet(const char* name)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
// Returns true if successful, false if otherwise.
bool CBasicExcel::DeleteWorksheet(const wchar_t* name)
{
	size_t maxWorksheets = worksheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
char* CBasicExcel::GetAnsiSheetName(size_t sheetIndex)
{
...
wchar_t* CBasicExcel::GetUnicodeSheetName(size_t sheetIndex)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
bool CBasicExcel::GetSheetName(size_t sheetIndex, char* name)
{
	if (!(workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1))
	{
...
bool CBasicExcel::GetSheetName(size_t sheetIndex, wchar_t* name)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CBasicExcel::RenameWorksheet(size_t sheetIndex, const char* to)
{
	size_t maxWorksheets = yesheets_.size();
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CBasicExcel::RenameWorksheet(size_t sheetIndex, const wchar_t* to)
{
	size_t maxWorksheets = yesheets_.size();
	if (sheetIndex < maxWorksheets)
	{
...
// Returns true if successful, false if otherwise.
bool CBasicExcel::RenameWorksheet(const char* from, const char* to)
{
	size_t maxWorksheets = yesheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
// Returns true if successful, false if otherwise.
bool CBasicExcel::RenameWorksheet(const wchar_t* from, const wchar_t* to)
{
	size_t maxWorksheets = worksheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
size_t CBasicExcel::Read(const char* data, size_t dataSize)
{
	size_t bytesRead = 0;
	BOF bof;
	short code;
...
size_t CBasicExcel::Write(char* data)
{
	size_t bytesWritten = 0;
...
void CBasicExcel::AdjustStreamPositions()
{
...
void CBasicExcel::AdjustBoundSheetBOFPositions()
{
...
void CBasicExcel::AdjustDBCellPositions()
{
	size_t offset = workbook_.RecordSize();
	size_t maxSheets = worksheets_.size();
...
void CBasicExcel::AdjustExtSSTPositions()
{
...
void CBasicExcel::UpdateYExcelWorksheet()
{
	size_t maxWorksheets = worksheets_.size();
	yesheets_.clear();
	...
	for (size_t i=0; i<maxWorksheets; ++i)
	{
		yesheets_.push_back(CBasicExcelWorksheet(this,i));
	}
}

// Update worksheets_ using information from yesheets_.
void CBasicExcel::UpdateWorksheets()
{
	// Constants.
	const size_t maxWorksheets = yesheets_.size();
	CWorksheet::CellTable::RowBlock rowBlock;
	CWorksheet::CellTable::RowBlock::CellBlock cellBlock;
	CWorksheet::CellTable::RowBlock::Row row;
	CWorksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK xfrk;
	LargeString largeString;
	...
	for (size_t s=0; s<maxWorksheets; ++s)
	{
		...
		vector<CWorksheet::CellTable::RowBlock>& rRowBlocks = worksheets_[s].cellTable_.rowBlocks_;
		vector<CWorksheet::CellTable::RowBlock::CellBlock>* pCellBlocks;
		CWorksheet::CellTable::RowBlock::CellBlock* pCell;
		rRowBlocks.resize(maxRows/32 + (maxRows%32 != 0 ? 1 : 0));
		for (size_t r=0, curRowBlock=0; r<maxRows; ++r)
		{
			...
			{
				CBasicExcelCell* cell = yesheets_[s].GetCell(r,c);
				int cellType = cell->Type();
				if (cellType != CBasicExcelCell::UNDEFINED)	// Current cell contains some data
				{		
					...
					if (newRow)
					{
						...
						CWorksheet::CellTable::RowBlock& rRowBlock = rRowBlocks[curRowBlock-1];
					...
					switch(cellType)
					{
						case CBasicExcelCell::INT:
						{
							// Check whether it is a single cell or range of cells.
							size_t cl = c + 1;
							for (; cl<maxCols; ++cl)
							{
								CBasicExcelCell* cellNext = yesheets_[s].GetCell(r,cl);
								if (cellNext->Type()==CBasicExcelCell::UNDEFINED ||
...
/************************************************************************************************************/
CBasicExcelWorksheet::CBasicExcelWorksheet(CBasicExcel* excel, size_t sheetIndex) : 
...
// Returns 0 if name is in Unicode format.
char* CBasicExcelWorksheet::GetAnsiSheetName()
{
...
// Get the current worksheet name.
// Returns 0 if name is in Ansi format.
wchar_t* CBasicExcelWorksheet::GetUnicodeSheetName()
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Get the current worksheet name.
// Returns false if name is in Unicode format.
bool CBasicExcelWorksheet::GetSheetName(char* name)
{
	if (!(excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1))
	{
...
// Get the current worksheet name.
// Returns false if name is in Ansi format.
bool CBasicExcelWorksheet::GetSheetName(wchar_t* name)
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Returns true if successful, false if otherwise.
bool CBasicExcelWorksheet::Rename(const char* to)
{
...
// Returns true if successful, false if otherwise.
bool CBasicExcelWorksheet::Rename(const wchar_t* to)
{
	size_t maxWorksheets = excel_->workbook_.boundSheets_.size();
	for (size_t i=0; i<maxWorksheets; ++i)
	{
...
void CBasicExcelWorksheet::Print(ostream& os, char delimiter, char textQualifier)
{
	for (size_t r=0; r<maxRows_; ++r)
	{
		for (size_t c=0; c<maxCols_; ++c)
		{
			CBasicExcelCell* cell = GetCell(r,c);
			switch (cell->Type())
			{
				case CBasicExcelCell::UNDEFINED:
					break;
...
size_t CBasicExcelWorksheet::GetTotalRows()
{
	return maxRows_;
}

// Total number of columns in current Excel worksheet.
size_t CBasicExcelWorksheet::GetTotalCols()
{
	return maxCols_;
}
...
CBasicExcelCell* CBasicExcelWorksheet::GetCell(size_t row, size_t col)	
{
	...
	if (row>=maxRows_) 
	{
		...
		maxRows_ = row + 1;
		cells_.resize(maxRows_, vector<CBasicExcelCell>(maxCols_));
	}
...
bool CBasicExcelWorksheet::EraseCell(size_t row, size_t col)
{
...
void CBasicExcelWorksheet::UpdateCells()
{
	// Define some reference
	CWorksheet::Dimensions& dimension = excel_->worksheets_[sheetIndex_].dimensions_;
	vector<CWorksheet::CellTable::RowBlock>& rRowBlocks = excel_->worksheets_[sheetIndex_].cellTable_.rowBlocks_;
	...
	vector<CBasicExcelCell> cellCol(maxCols_);
	cells_.resize(maxRows_, cellCol);

	size_t maxRowBlocks = rRowBlocks.size();
	for (size_t i=0; i<maxRowBlocks; ++i)
	{
		vector<CWorksheet::CellTable::RowBlock::CellBlock>& rCellBlocks = rRowBlocks[i].cellBlocks_;
...
bool CBasicExcelCell::Get(double& val) const 
{
	if (type_ == DOUBLE)
	{
		val = dval_;
...
bool CBasicExcelCell::Get(char* str) const 
{
	if (type_ == STRING)
	{
		if (str_.empty()) *str = '\0';
...
bool CBasicExcelCell::Get(wchar_t* str) const 
{
	if (type_ == WSTRING)
	{
...
size_t CBasicExcelCell::GetStringLength() const
{
...
int CBasicExcelCell::GetInteger() const
{
	int val;
...
double CBasicExcelCell::GetDouble() const
{
	double val;
	if (Get(val)) return val;
	else return 0.0;
...
const char* CBasicExcelCell::GetString() const
{
...
const wchar_t* CBasicExcelCell::GetWString() const
{
...
void CBasicExcelCell::Set(int val) 
{
	SetInteger(val);
}

// Set content of current Excel cell to a double.
void CBasicExcelCell::Set(double val) 
{
	SetDouble(val);
}

// Set content of current Excel cell to an ANSI string.
void CBasicExcelCell::Set(const char* str) 
{
	SetString(str);
}

// Set content of current Excel cell to an Unicode string.
void CBasicExcelCell::Set(const wchar_t* str)	
{
	SetWString(str);
}

// Set content of current Excel cell to an integer.
void CBasicExcelCell::SetInteger(int val) 
{
	type_ = INT; 
	ival_ = val;
}
...
// Set content of current Excel cell to a double.
void CBasicExcelCell::SetDouble(double val) 
{
	type_ = DOUBLE; 
	dval_ = val;
}
...
// Set content of current Excel cell to an ANSI string.
void CBasicExcelCell::SetString(const char* str) 
{
...
// Set content of current Excel cell to an Unicode string.
void CBasicExcelCell::SetWString(const wchar_t* str)	
{
...
void CBasicExcelCell::EraseContents()
{
	type_ = UNDEFINED;
	str_.clear();
	wstr_.clear();
...
ostream& operator<<(ostream& os, const CBasicExcelCell& cell)
{
	switch (cell.Type())
	{
		case CBasicExcelCell::UNDEFINED:
