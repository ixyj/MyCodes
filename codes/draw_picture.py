...
plt.legend(bbox_to_anchor=(1.01,1.0),borderaxespad = 0.) #设置图例位置
plt.grid()#添加网格

gca = plt.gca()
gca.xaxis.set_major_locator(ticker.MultipleLocator(5)) #x轴每隔5显示一个刻度
gca.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))#x轴显示时间
gca.yaxis.set_major_locator(ticker.MultipleLocator(2000))
plt.gcf().autofmt_xdate()#x轴旋转显示时间

fig.subplots_adjust(right=0.8)   #图片右面边缘大小
