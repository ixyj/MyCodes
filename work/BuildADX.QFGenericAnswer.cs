    ...
    class Program
    {
        ...
        private static void GenerateQFDataFile(string inFile, string[] outFiles)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                var normilized = Normalize(WordBreaker.BreakWords(line, "zh-CN", false));
                if (string.IsNullOrWhiteSpace(normilized))
                {
                    Console.WriteLine("Empty normilized line: " + normilized + "\tRaw line: " + line);
                }
                else
                {
                    writer.Write("</Trigger><Trigger>");
                    writer.Write(normilized); 
                }
